...
type TabId = 'terminal' | 'desktop' | 'chat'

// Three ways in, self-selected by where you work, each previewing its coverage
// with the real product logos.
const TABS: ReadonlyArray<{ id: TabId; label: string; icons: IconComponent[] }> = [
...
/**
 * Open on the tab this visitor can actually act on.
 *
 * Terminal was the fixed default, which is wrong for most of the people Skillet
 * is trying to reach: on a phone, both Terminal and Desktop App are dead ends,
 * and Chat is the only way in. Chat is also the delivery mode that needs nothing
 * installed at all, which is exactly the surface growing fastest.
 *
 * The server render has no UA, so it opens on Terminal and corrects after
 * hydration. Anyone whose right answer is Terminal never sees a change.
 */
function defaultTab(os: InstallPlatform | null): TabId {
  if (os === 'mobile') return 'chat'
  if (os === 'mac' || os === 'windows') return 'desktop'
  return 'terminal'
}

/**
 * The three ways to get Skillet onto a machine, in one place.
 *
 * Shared rather than duplicated: install used to be told three times (the
 * homepage box, the skill page's panel, the kit page's bar), which is how
 * three tellings drift into three different products. `compact` is the same
 * picker with the chrome taken off, for use inside another surface.
 */
export function InstallPicker({
  ...
  /** Strip the outer width, heading row spacing, and reserved height. */
  compact?: boolean
  /** A signed-in viewer can turn on their own MCP link; a stranger can only read
   *  about it. Changes what the Chat tab offers. */
  signedIn?: boolean
  /** Rendered left of the tab row. Omitted in compact use, where the surface
   *  around it already said what this is. */
  heading?: string
}) {
  const reduce = useReducedMotion()
  const [os, setOs] = useState<InstallPlatform | null>(null)
  const [tab, setTab] = useState<TabId>('terminal')
  // A tab the visitor picked outranks the platform guess arriving late.
  const [chosen, setChosen] = useState(false)
  const [pm, setPm] = useState('npx')
  // Two pickers on one page would otherwise share a layoutId and slide their
  // underlines into each other.
  ...
  useEffect(() => {
    const detected = detectInstallPlatform(navigator.userAgent, navigator.platform)
    setOs(detected)
    setTab((current) => (chosen ? current : defaultTab(detected)))
    // `chosen` is read, never tracked: this runs once, and a later pick is
    // guarded by the setter above.
  ...
  return (
    <div
      className={compact ? 'w-full max-w-[520px]' : 'mx-auto mt-10 w-full max-w-[420px] lg:mx-0'}
    >
      <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-2">
        {heading && <h2 className="pb-1 text-base font-semibold text-(--ink)">{heading}</h2>}
        <div className={`flex items-center gap-5 ${heading ? '' : 'w-full'}`}>
          {TABS.map((t) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <motion.span
                    layoutId={`install-tab-underline-${underlineId}`}
      ...
      {/* Reserved height only where the picker stands alone. Inside another
          surface the bar around it is already springing its own height, and a
          second fixed block would fight it. */}
      <div className={`mt-1 text-left ${compact ? '' : 'min-h-[96px]'}`}>
        {/* mode="wait" so the outgoing panel is gone before the next arrives.
            Opacity only: nothing needs to measure or move. */}
          ...
          >
            {tab === 'terminal' && (
              <>
                <CommandBlock
                  ...
                  wrap
                  bare
                  // Standing alone the block lifts off the page with --surface.
                  // Inside the kit page's notice it would be white on white, so
                  // it insets against the card instead.
                  className={compact ? 'bg-(--bg)' : 'bg-(--surface)'}
                />
            ...
            {tab === 'desktop' && (
              <div className="flex flex-col items-start gap-2.5">
                {/* Standing alone this line is the only thing saying what the app
                    does. Inside a card that already has a headline and a sub, it
                    is the same sentence a third time. */}
                {!compact && (
            ...
            {tab === 'chat' && <ChatPanel signedIn={signedIn} compact={compact} />}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
...
/**
 * Chat is the only way in that installs nothing, and the only one that works on
 * a phone. What it should offer depends on who is asking.
 *
 * A stranger can only read about it, so it points at the runtime guides. Someone
 * signed in owns an MCP link already, off by default, one switch away: for them
 * the move is turning it on, not reading how it works.
 */
function ChatPanel({ signedIn, compact = false }: { signedIn: boolean; compact?: boolean }) {
  if (signedIn) {
    return (
      <div className="flex flex-col items-start gap-2.5">
        {/* Kept even when compact: unlike the other two tabs, this one says
            something the card above it does not, which is that this path
            installs nothing at all. */}
        <p className="text-xs text-(--ink-2)">
          {compact
            ? 'Works in ChatGPT and Claude.ai. Nothing to install.'
            : 'Connect ChatGPT, Claude.ai, or anything that speaks MCP. Nothing to install.'}
        </p>
        <div className="flex flex-wrap items-center gap-2">
          <Link href="/settings#mcp" className={ACTION}>
            Turn on your MCP link
          </Link>
          <Link
            href="/docs/mcp"
            className="text-xs font-medium text-(--ink-2) underline decoration-(--line) underline-offset-2 transition-colors hover:text-(--ink)"
          >
            How it works
          </Link>
        </div>
...
/**
 * The three ways in as one row of doors, instead of tabs over a panel.
 *
 * The tabbed picker asks you to choose before you act: select Terminal, read a
 * line, then press something. Inside a card that already has a headline and a
 * sub, that is two steps and five layers for what is really three options. Worse,
 * tab controls look like buttons while doing nothing, so the loudest things on
 * the card sit above the only element that acts.
 *
 * Here every visible control is a door. One of them is emphasised, chosen from
 * the platform, because there is normally exactly one right answer and we
 * already know it: an app download on Mac and Windows, a command on Linux, and
 * on a phone only MCP, because the other two are dead ends there.
 *
 * The package-manager switcher does not come along. `npx` works for everyone,
 * and someone who wants `bun` will not be stopped by it. The full picker keeps
 * the switcher for the homepage, where there is room to browse rather than act.
 */
