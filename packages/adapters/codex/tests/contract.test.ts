...
describe("codex adapter contract", () => {
  ...
  beforeEach(async () => {
    tmpBase = join(tmpdir(), `skillet-codex-${randomBytes(4).toString("hex")}`);
...
describe("codex project adapter contract", () => {
  let tmpProject: string;
  let adapter: ReturnType<typeof createProjectAdapter>;

  beforeEach(async () => {
    tmpProject = join(tmpdir(), `skillet-codex-proj-${randomBytes(4).toString("hex")}`);
  ...
  it("adapter name is 'codex-project'", () => {
    expect(adapter.name).toBe("codex-project");
  });
...
describe("findProjectRoot", () => {
  ...
  it("does NOT treat a bare package.json as a project root (dropped 2026-07)", async () => {
    // Codex keys project scope on the repo root (.git) — a Node package with
    // no git dir is global-scoped, matching Codex, not project-scoped.
  ...
  it("returns null when no marker is found", async () => {
    ...
    try {
      // Bounded at the fixture, so this asserts null outright. It used to only
      // check `string | null` — which passes no matter what the walk does —
      // because an unbounded walk can find whatever sits above tmpdir.
      expect(await findProjectRoot(noMarker, noMarker)).toBeNull();
    } finally {
      await rm(noMarker, { recursive: true, force: true });
    }
  });

  it("stops at the boundary instead of climbing past it", async () => {
    // The marker is ABOVE the boundary: an unbounded walk finds it, a bounded
    // one must not.
    await mkdir(join(tmpRoot, ".git"));
    const sub = join(tmpRoot, "src", "lib");
    await mkdir(sub, { recursive: true });
    expect(await findProjectRoot(sub)).toBe(tmpRoot);
    expect(await findProjectRoot(sub, sub)).toBeNull();
    // The boundary itself is searched, not skipped.
    expect(await findProjectRoot(sub, tmpRoot)).toBe(tmpRoot);
  });
});
